...
class Game extends Component {
  ...
  render() {
	...
	const moves = history.map((step,move) => {
		let desc = winner ? "Next game" : "Restart";
		if(move){
			const player = step.isX ? playerX : playerO;
			desc = player + " @ " +  + step.row + ":" + step.col + " | Go to move #" + move;
		}
		return (
			<li key={move} className={move === this.state.stepNumber ? 'bold-text' : ''}>
